<template>
    <div class="model">
        <div class="form" @keyup.esc="cancelEvent">
            ...
            <div v-if="mode=='update'" class="form-group">  <!-- 업데이트 폼 -->
                <label>일련번호</label>
...
export default {
    name : "contactForm",
    props : {
        mode : { type:String, default:'add' },   //props로 mode를 전달받아 default(add)면 추가폼, update면 수정폼으로 변경
        contact : {
            type : Object,
            default : function() {
                return { no:'', name:'',tel:'',address:'',photo:'' }  //추가폼에서는 값이 없어야 하므로 default는 공백으로 설정
            }
        }
    },
    mounted : function() {
        this.$refs.name.focus()   //마운트 된 후, ref로 이름입력 태그로 focus 설정
    },
    computed : {
        btnText : function() {
...
<style scoped>
.model { display: block; position: fixed; z-index: 1;   /* 모달 폼 적용 */
...
.form input, textarea, select { box-sizing:border-box; border:1px solid #BEBEBE;
...
.form .heading { background: #33A17F; font-weight: 300; text-align:left; padding: 20px;
                color:#fff; margin:5px 0 30px 0; padding:10px; min-width:200px; max-width:400px; }                                                             
</style>