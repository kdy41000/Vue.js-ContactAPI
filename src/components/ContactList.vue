...
export default {
    ...
    props : [ 'contactlist' ],
    computed : {  //계산형 속성
        totalpage : function() {  //전체 페이지 개수 return
            return Math.floor((this.contactlist.totalcount-1) / this.contactlist.pagesize)+1;
        }
    },
    watch : {   //값이 입력되면 실행되는 옵션
        ['contactlist.pageno'] : function() { //새로운 연락처 추가 시 첫 번째 페이지로 이동시키는 기능
...
<style scoped>
.addnew { margin:10px auto; max-width: 820px; min-width: 820px; padding:40px 0px 0px 0px;
        text-align:left; }
#example { margin:10px auto; max-width:820px; min-width: 820px; padding:0px; position:relative;
         font:13px "verdana"; }
